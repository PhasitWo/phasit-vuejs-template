...
const actions: ActionTree<LibraryStateType, any> = {
  async getBookList(context, params?: SearchParams) {
    let requestURL = '/search.json?title=the&limit=12'
    if (params)
      requestURL = `/search.json?title=${params.query}&page=${params.page}&limit=${params.itemPerPage}`
    try {
